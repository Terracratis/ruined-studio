<?php
	...
	$SQL = $DB->prepare("SELECT * FROM dish_categories WHERE is_draft = ? ORDER BY category_id DESC LIMIT 1");
	$SQL->execute(array('1'));
	$draft = $SQL->fetch();
	if (!$draft)
		{
			$SQL = $DB->prepare("INSERT INTO dish_categories (is_draft) VALUES (?)");
			$SQL->execute(array('1'));
			$id = $DB->lastInsertId();
			$SQL = $DB->prepare("UPDATE dish_categories SET name_key = ? WHERE is_draft = ? AND category_id = ? LIMIT 1");
			$SQL->execute(array('category_'.$id,'1',$id));
			$SQL = $DB->prepare("SELECT * FROM dish_categories WHERE is_draft = ? AND category_id = ? LIMIT 1");
...
function rsizer()
		...
		$("#Save").click(function(){
			showAlert("Išsaugoma.");
			$.post("{ROOT_STUDIO}/editor.php",{id: "menu-item/category-save",value: "<?=$draft['category_id']?>"}).done(function(data){if(data == "OK"){window.location="{URL}/menu-categories"}showAlert(data);});
		});
	});
